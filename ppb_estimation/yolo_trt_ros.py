...
def calculate_distance(tstl_list, xycar_image):
    ...
    for tstl in tstl_list:
        ...
        if tstl_width_in_frame != 0:
            ...
            cv2.putText(
                xycar_image, str(round(Distance, 3)) + "CM", (x1, y1 - 20), fonts, 0.5, (WHITE), 1
            )
            cv2.putText(
                xycar_image, str(round(x_dist, 3)) + str(round(y_dist, 3)), (x1, y1), fonts, 0.5, (RED), 1
            )
    return x_y_dist_list
...
while not rospy.is_shutdown():
    ...
    x_y_dist_list = calculate_distance(bbox_list, xycar_image)
    try:
        for i in range(len(x_y_dist_list)):
            print("detect", x_y_dist_list[i])
    except:
        pass
